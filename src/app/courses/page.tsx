            ...
            <div className="max-w-7xl mx-auto text-center mt-30">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 justify-center">
                {careerPaths.map((path: CareerPath) => {
                  ...
                  return (
                    <div key={path.title} className={`${glassEffect} p-8 hover:scale-105 transition-transform duration-500 animate-slide-up ${path.themeColor === 'blue' ? 'hover:shadow-blue-500/40' :
                      ...
                      <div className={`${isComingSoon ? 'opacity-50 pointer-events-none' : ''}`}>
                        {!isComingSoon ? (
                          <button onClick={() => openCareerModal(path)} className={`px-6 py-3 rounded-xl transition cursor-pointer ${path.themeColor === 'blue' ? 'bg-blue-500 hover:bg-blue-600' :
          ...
          <main className="container mx-auto px-6 py-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {filteredCourses.map((course: Course) => (
                <div key={course.path} className={`${glassEffect} overflow-hidden hover:shadow-red-500/40 transition-all duration-500 transform hover:-translate-y-2 animate-slide-up flex flex-col h-full`}>
                  <Image
                    src={course.image}
                    alt={course.title}
                    width={500}
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <h3 className="text-2xl font-bold mb-2 text-gray-100">{course.title}</h3>
                    <p className="text-gray-400 mb-6">{course.description}</p>
                    <div className="flex justify-center mt-auto">
                      {["ccsa", "cna", "cass", "cbbe"].includes(course.path) ? (
                        <div className="w-60 bg-gray-700 text-gray-300 font-semibold py-3 px-4 rounded-[20px] text-center cursor-not-allowed">
                          Coming Soon
                        </div>
                      ) : (
                        <button
                          onClick={() => handleViewDetails(course.path)}
                          className="w-60 bg-red-600 hover:bg-blue-600 text-white font-bold py-3 px-4 rounded-[20px] transition-colors shadow-md hover:shadow-lg shadow-red-500/30 text-center cursor-pointer"
                        >
                          View Details
                        </button>
                      )}
