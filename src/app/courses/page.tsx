...
<div className="mb-23 flex flex-wrap justify-center gap-4 animate-fade-up">
  {categories.map((category: string) => (
    <button
      key={category}
      onClick={() => setFilter(category)}
      className={`backdrop-blur-sm border border-slate-700 rounded-full px-6 py-3 font-semibold transition-all duration-300 cursor-pointer
      shadow-[inset_-3px_-1px_9px_rgba(255,255,255,0.5),_inset_10px_10px_20px_rgba(0,0,0,0.2),_0_4px_30px_rgba(0,0,0,0.1)]
      ${
        filter === category
          ? "text-white scale-105 ring-2 ring-red-500 bg-red-600/40"
          : "text-gray-300 hover:scale-105 hover:bg-white/5"
      }`}
    >
      {category}
    </button>
